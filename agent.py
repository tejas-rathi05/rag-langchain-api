...
prompt = ChatPromptTemplate.from_messages([
    ("system", (
        ...
        "- 'search_document': Use this to answer questions about the content of uploaded documents.\n"
        "Always use 'search_document' to answer the user's question. "
        "After using a tool, the tool output will be provided back to you. "
        "When you have all the information you need, you MUST use the final_answer tool "
        "to provide a final answer to the user. Use tools to answer the user's CURRENT question, not previous questions."
    )),
...
@tool
async def search_document(query: str, top_k: int = 3) -> list[str]:
...
tools = [add, subtract, multiply, exponentiate, final_answer, search_document]
